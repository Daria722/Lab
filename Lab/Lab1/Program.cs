...
class Program
{
    ...
    static void Main(string[] args)
    {
       ...
       // user = (1, "Ivan");
        ...
        // Чтение данных
        ReadGeneticData("sequences0.txt");

        var commands = File.ReadAllLines("commands0.txt");

        outputLines.Add("Daria");
        ...
        File.WriteAllLines("genedata0.txt", outputLines);

        Console.WriteLine("Обработка завершена. Результаты записаны в genedata0.txt");
    }
        ...
        static void ReadGeneticData (string filename)
        {
            try
            {
                ...
                foreach (var line in lines)
                {
                    ...
                    if (parts.Length>=3)
                    {
                    string decodedAminoAcids = RLDecoding(parts[2].Trim());
                    var invalidChars = decodedAminoAcids.Where(c => !validAminoAcids.Contains(c)).Distinct();
                    if (invalidChars.Any())
                        Console.WriteLine($"В белке {parts[0].Trim()} найдены недопустимые символы: {string.Join(", ", invalidChars)}");
    ...
    static void DiffCommand(string p1, string p2)
    {
        ...
        {
            string decoded1 = RLDecoding(g1.amino_acids);
            string decoded2 = RLDecoding(g2.amino_acids);
            int diff = AminoDifference(decoded1, decoded2);
    ...
    static void ModeCommand(string pname)
    {
        ...
        {
            string decodedAminoAcids = RLDecoding(g.amino_acids);
            var (am, count) = ModeAminoAcid(decodedAminoAcids);
